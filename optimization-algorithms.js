// ============================================
// OPTIMIZATION ALGORITHMS - IMPROVED & CLEAN
// ============================================

// ============================================
// 1D OPTIMIZATION - INTEGER ONLY
// ============================================
...
class CuttingItem1D {
    constructor(id, length, quantity) {
        ...
        this.position = 0;
        this.barId = '';
        this.originalLength = Math.round(length);
...
class Bar1D {
    constructor(id, maxLength) {
        this.id = id;
        this.maxLength = Math.round(maxLength);
        this.items = [];
        this.usedLength = 0;
        this.remainingLength = Math.round(maxLength);
        this.efficiency = 0;
    ...
    placeItem(item) {
        if (this.canPlace(item)) {
            ...
            this.usedLength = Math.round(this.usedLength + item.length);
            this.remainingLength = Math.round(this.maxLength - this.usedLength);
            this.efficiency = Math.round((this.usedLength / this.maxLength) * 100);
            this.wastePercentage = Math.round((this.remainingLength / this.maxLength) * 100);
            return true;
        }
        return false;
    }
    ...
    getWaste() {
        return Math.round(this.remainingLength);
    }
}
    ...
    optimize(items, materialLength) {
        ...
        items.forEach(item => {
            for (let i = 0; i < item.quantity; i++) {
                ...
                allItems.push(new CuttingItem1D(itemId, Math.round(item.length), 1));
            }
        });
        ...
        switch (this.algorithm) {
            case 'best-fit':
                bars = this.bestFitAlgorithm(allItems, Math.round(materialLength));
                break;
            case 'worst-fit':
                bars = this.worstFitAlgorithm(allItems, Math.round(materialLength));
                break;
            default:
                bars = this.firstFitAlgorithm(allItems, Math.round(materialLength));
        }

        const totalBars = bars.length;
        const totalItemsPlaced = bars.reduce((sum, bar) => sum + bar.items.length, 0);
        
        const totalMaterialLength = Math.round(totalBars * materialLength);
        const totalUsedLength = Math.round(bars.reduce((sum, bar) => sum + bar.usedLength, 0));
        const totalWaste = Math.round(bars.reduce((sum, bar) => sum + bar.getWaste(), 0));
        const overallEfficiency = Math.round(totalMaterialLength > 0 ? (totalUsedLength / totalMaterialLength) * 100 : 0);
        ...
        return {
            ...
            totalWaste,
            overallEfficiency,
            executionTime: Math.round(Date.now() - startTime),
...
class Plate2D {
    constructor(id, width, height) {
        ...
        this.items = [];
        this.usedArea = 0;
        this.totalArea = Math.round(width * height);
        this.freeRects = [{ x: 0, y: 0, width: Math.round(width), height: Math.round(height) }];
    }
    ...
    getWasteArea() {
        return Math.round(this.totalArea - this.usedArea);
    }

    getWastePercentage() {
        return Math.round(this.totalArea > 0 ? (this.getWasteArea() / this.totalArea) * 100 : 0);
    }
    ...
    placeItem(item, x, y) {
        if (this.canPlace(item, x, y)) {
            ...
            item.placed = true;
            this.items.push(item);
            this.usedArea = Math.round(this.usedArea + item.area);
    ...
    splitFreeRect(freeRect, placedItem) {
        ...
        if (placedItem.x > freeRect.x) {
            newRects.push({
                x: freeRect.x,
                y: freeRect.y,
                width: Math.round(placedItem.x - freeRect.x),
        ...
        if (placedItem.x + placedItem.width < freeRect.x + freeRect.width) {
            newRects.push({
                x: Math.round(placedItem.x + placedItem.width),
                y: freeRect.y,
                width: Math.round(freeRect.x + freeRect.width - (placedItem.x + placedItem.width)),
                height: freeRect.height
            });
        }
        ...
        if (placedItem.y > freeRect.y) {
            newRects.push({
                ...
                height: Math.round(placedItem.y - freeRect.y)
            });
        }
        ...
        if (placedItem.y + placedItem.height < freeRect.y + freeRect.height) {
            newRects.push({
                x: freeRect.x,
                y: Math.round(placedItem.y + placedItem.height),
                width: freeRect.width,
                height: Math.round(freeRect.y + freeRect.height - (placedItem.y + placedItem.height))
            });
        }
...
class PlateOptimizer2D {
    ...
    optimize(items, plateWidth, plateHeight) {
        ...
        switch (this.algorithm) {
            case 'GUILLOTINE':
            case 'MAXRECTS':
                plates = this.bestFitBinPack(allItems, Math.round(plateWidth), Math.round(plateHeight));
                break;
            default:
                plates = this.simpleBinPack(allItems, Math.round(plateWidth), Math.round(plateHeight));
        }

        const totalPlates = plates.length;
        const totalItemsPlaced = plates.reduce((sum, p) => sum + p.items.length, 0);
        
        const totalArea = Math.round(plates.reduce((sum, p) => sum + p.totalArea, 0));
        const totalUsedArea = Math.round(plates.reduce((sum, p) => sum + p.usedArea, 0));
        const overallEfficiency = Math.round(totalArea > 0 ? (totalUsedArea / totalArea) * 100 : 0);
        ...
        return {
            ...
            totalUsedArea,
            overallEfficiency,
            unplacedItems: Math.round(allItems.length - totalItemsPlaced),
            executionTime: Math.round(Date.now() - startTime),
            algorithm: this.algorithm,
        };
    }
    ...
    findPositionSimple(item, plate) {
        for (let y = 0; y <= plate.height - item.height; y++) {
            for (let x = 0; x <= plate.width - item.width; x++) {
                if (plate.canPlace(item, x, y)) {
                    return { x: Math.round(x), y: Math.round(y), rotated: false };
                }
            }
        }
        ...
        if (item.canRotate) {
            ...
            for (let y = 0; y <= plate.height - item.height; y++) {
                for (let x = 0; x <= plate.width - item.width; x++) {
                    if (plate.canPlace(item, x, y)) {
                        return { x: Math.round(x), y: Math.round(y), rotated: true };
                    }
                }
            }
            item.rotate();
        ...
        for (let rotated = 0; rotated < (item.canRotate ? 2 : 1); rotated++) {
            ...
            for (const rect of plate.freeRects) {
                if (currentItem.width <= rect.width && currentItem.height <= rect.height) {
                    const score = Math.round(rect.width * rect.height - currentItem.area);
...
class CustomOptimizer {
    ...
    calculatePatternEfficiency(bars, items) {
        ...
        items.forEach(item => {
            if (item.pattern === 'A') {
                ...
                patternTotals.B.totalLength += parseInt(item.originalLength) * parseInt(item.quantity);
            }
        });
        ...
        bars.forEach(bar => {
            bar.items.forEach(item => {
                ...
                if (pattern === 'A') {
                    patternTotals.A.usedLength += parseInt(item.originalLength);
                } else if (pattern === 'B') {
                    patternTotals.B.usedLength += parseInt(item.originalLength);
                }
            });
        });
        ...
        const efficiencyB = patternTotals.B.totalLength > 0 ? 
            Math.round((patternTotals.B.usedLength / patternTotals.B.totalLength) * 100) : 0;
        
        return {
            A: { 
                totalLength: parseInt(patternTotals.A.totalLength),
                usedLength: parseInt(patternTotals.A.usedLength),
                efficiency: efficiencyA
            },
            B: { 
                totalLength: parseInt(patternTotals.B.totalLength),
                usedLength: parseInt(patternTotals.B.usedLength),
    ...
    optimizeFFCA01(params, materialLength = 6000) {
        ...
        const integerItems = customItems.map(item => ({
            ...
            originalLength: parseInt(item.originalLength)
        }));
        ...
        result.totalWaste = parseInt(result.totalWaste || 0);
        ...
        if (result.bars && Array.isArray(result.bars)) {
            result.bars.forEach(bar => {
                ...
                bar.remainingLength = parseInt(bar.remainingLength || 0);
                bar.efficiency = Math.round(bar.efficiency || 0);
                bar.wastePercentage = Math.round(bar.wastePercentage || 0);
                
                if (bar.items && Array.isArray(bar.items)) {
                    bar.items.forEach(item => {
                        item.originalLength = parseInt(item.originalLength || 0);
        ...
        result.customParams = {
            ...
            bigRingA: parseInt(params.bigRingA || 0),
            smallRingB: parseInt(params.smallRingB || 0),
            bigRingB: parseInt(params.bigRingB || 0),
            multiplier: parseInt(params.multiplier || 1),
            kerfWidth: parseInt(params.kerfWidth || 0)
        };
        ...
        const totalPatterns = {
            ...
            B: integerItems.filter(item => item.pattern === 'B').reduce((sum, item) => sum + parseInt(item.quantity), 0)
        };
        
        result.customStats = {
            totalCuts: parseInt(totalCuts),
            totalPatterns: {
                A: parseInt(totalPatterns.A),
                B: parseInt(totalPatterns.B)
            },
            cutsPerPattern: Math.round(totalCuts / 2),
